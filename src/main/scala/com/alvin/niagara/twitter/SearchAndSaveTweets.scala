...
object SearchAndSaveTweets extends App with FileSupport with Config {
  ...
  def searchTweets(query: String, max_id: Option[Long] = None): Future[Seq[Tweet]] = {
    ...
    client.searchTweet(query, count = 100, result_type = ResultType.Recent, max_id = max_id)
      .flatMap { ratedData =>
        ...
        if (tweets.nonEmpty)
          searchTweets(query, nextMaxId).map(_ ++ tweets)
        else
